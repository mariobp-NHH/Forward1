...
from webse.gd_course_app_calculator_HVL_2023_student1.forms import AddRecordForm
from webse.models import EmissionsGD
from flask_login import login_user, current_user, logout_user, login_required

gd_course_app_calculator_HVL_2023_student1 = Blueprint('gd_course_app_calculator_HVL_2023_student1', __name__)
...
@gd_course_app_calculator_HVL_2023_student1.route("/green_digitalization_course/app_calculator/HVL/2023/student1", methods=['GET', 'POST'])
@login_required
def app_calculator_entry():
    ...
    if form.validate_on_submit():
        ...
        emissions = EmissionsGD(kms=kms, transport=transport, fuel=fuel, co2=co2, ch4=ch4, total=total, author=current_user, 
                student='student1', institution='none', year=2023)
        db.session.add(emissions)
        db.session.commit()
        return redirect(url_for('gd_course_app_calculator_HVL_2023_student1.app_calculator_table_graph'))
    return render_template('gd_course/app_calculator_HVL_2023_student1/app_calculator.html', title='Carbon Emissions Calculator', legend=legend, paragraph=paragraph, form=form)

@gd_course_app_calculator_HVL_2023_student1.route("/green_digitalization_course/app_calculator/HVL/2023/student1/table_graph", methods=['GET', 'POST'])
@login_required
def app_calculator_table_graph():
    #Table
    entries = EmissionsGD.query.filter_by(author=current_user). \
        filter(EmissionsGD.date> (datetime.now() - timedelta(days=5))).filter(EmissionsGD.student=='student1').\
    ...
    emissions_by_transport = db.session.query(db.func.sum(EmissionsGD.total), EmissionsGD.transport). \
        filter(EmissionsGD.date > (datetime.now() - timedelta(days=5))).filter(EmissionsGD.student=='student1').filter_by(author=current_user). \
    ...
    return render_template('gd_course/app_calculator_HVL_2023_student1/app_calculator_table_graph.html', entries=entries,
...
@gd_course_app_calculator_HVL_2023_student1.route('/green_digitalization_course/delete-emission/<int:entry_id>')
...
@gd_course_app_calculator_HVL_2023_student1.route('/green_digitalization_course/fuel_type/<transport>')
